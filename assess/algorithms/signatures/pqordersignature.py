...
class PQOrderSignature(Signature):
    """
    The PQOrder Signature creates a signature that is required to eliminate
    permutations within data. It is dependent on infinite-length encoding in p
    dimension and finite-length encoding in Q dimension. Furthermore, it
    performs an ordering in Q dimension for width + 1 nodes. Hence, only width
    nodes are encoded. The ordering also includes the actual anchor node.
    ...
    def prepare_signature(self, node, parent):
        ordered_nodes = [node.name] + list(self.sibling_generator(
            node, self._width + 1))
