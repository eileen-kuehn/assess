"""
This module offers implementation for SimpleDistance. There are different methods
that are implemented. One does start with a distance of 0 and the other with a
distance initialised with the number of nodes within the prototypes.
"""
...
class SimpleDistance(Distance):
    """
    SimpleDistance class implements an exact measure by initialising the distance
    with the number of nodes inside the prototype. For each signature (that has
    not been checked before) the base distance is initialised with one. If the
    signature does match a prototype, the distance is set to -1. This distance
    measure has the property, that it is rising and falling. Maybe not adequate
    for anomaly detection but it delivers exact results without considering a
    final step.
...
class SimpleDistance2(Distance):
    ...
    def finish_distance(self, prototypes, signature_prototypes):
        result_dict = [dict(zip(prototypes, [0] * len(prototypes))) for _
                       in range(self.signature_count)]
        ...
        for prototype in prototypes:
            ...
            for index, node_count in enumerate(node_counts):
                result_dict[index][prototype] = \
                    node_count - (len(self._measured_nodes[index])
        ...
        return [[value] for monitoring_result in self._monitoring_results_dict for
                value in monitoring_result.values()]
