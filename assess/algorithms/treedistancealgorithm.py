...
class TreeDistanceAlgorithm(object):
    """
    The class TreeDistanceAlgorithm creates the API for different algorithms
    calculating the distance between trees. The most important methods to consider
    for implementation of a new distance algorithm are
    ...
    * _update_distance
    ...
    def prototypes(self):
        """
        Property that gives access to the prototypes being used for distance
        calculations.
    ...
    def prototypes(self, value=None):
        """
        Setter method to set the current list of prototypes to be used for
    ...
    def signature_prototypes(self):
        """
        Method that returns the signatures of the current prototypes being used
    ...
    def cluster_representatives(self, signature_prototypes=None, prototypes=None):
        """
        Method that sets the signatures and cluster names for cluster
        represenatives to check.
        ...
        if isinstance(signature_prototypes, list):
            self._signature_prototypes = \
                self._signature.prototype_signature_cache_class.\
    ...
    def tree_node_counts(self, signature=False):
        """
        Returns the list of count of nodes for monitoring tree per prototype.
        If signature is True, the count of nodes based on signature is used.
        Signature defaults to False.

        Format is like this: [ve1, ..., ven]

        :param signature: Determines if node count depends on signature,
            defaults to False
    ...
    def prototype_node_counts(self, signature=False):
        """
        Returns the count of nodes per prototype tree. If signature is True,
        the count for converted prototypes based on given signature is returned.
        Signature defaults to False.
        ...
        try:
            return [[prototype.node_count() for prototype in self._prototypes] for
    ...
    def tree_event_counts(self):
        try:
            ...
            try:
                ...
                event_counts = 0
        return [event_counts for _ in range(self._signature.count)] \
            if event_counts > 0 else []
    ...
    def event_counts(self, by_event=False):
        """
        Method returns a list containing the current events considered from the
        monitoring tree by prototype.
    ...
    def start_tree(self, maxlen=None, **kwargs):
        """
        Method that should be called before a new event stream is started.
        It takes care on initialising things.
    ...
    def finish_tree(self):
        """
        Method that should be called after the event stream has been finished.
        Some of the algorithms might rely on this method to be called.
    ...
    def add_events(self, eventgenerator, **kwargs):
        """
        Convenience method that takes an event generator and calls method
        add_event for each event that is yielded.
    ...
    def add_event(self, event, **kwargs):
        """
        Method to add an event. For each event the actual distance from the stream
        object to different prototypes are calculated. The calculated distance is
        returned.

        Format that can be expected:
            [[v1p1e1, ..., vnpne1], ..., [v1p1en, ..., vnpnen]]

        Attention: This format is put into a list, because of empty nodes that
        might be required to put into the current tree.
        ...
        if isinstance(event, ProcessStartEvent):
            ...
            for signature in signatures:
                if self.supported.get(ProcessStartEvent, False):
                    ...
                    result.append(self.update_distance(
                        start_event, signature, **kwargs))
                    event.signature.append(signature)
    ...
    def update_distance(self, event, signature, **kwargs):
        """
        Method to update the current distance based on the received event and
    ...
    def __repr__(self):
        return "%s (cache_statistics=%s, supported=%s)" % \
               (self.__class__.__name__, self._cache_statistics.__name__
                if self._cache_statistics else None,
