...
class RandomGenerator(object):
    """
    The RandomGenerator initializes a prototypes and generates random events based
    on the generated prototype. It can be differently parameterised to test for
    different things.

    Attention: The RandomGenerator currently should not be used for stuff build
    on tree topology. Currently only the depth of 1 is created. So all nodes are
    below a single root node.
    """
    def __init__(self, prototype_node_count=100, tree_node_count=100,
                 relative_repetition=0, relative_matching=0.5, seed=None):
    ...
    def prototype(self):
        ...
        if self._prototype is None:
            prototype = Prototype()
            root = prototype.add_node(
                name=id_generator(size=6), tme=0, exit_tme=0, pid=1, ppid=0)
            for i in range(self._prototype_node_count):
                ...
                prototype.add_node(
                    name=node_name, parent=root, tme=0, exit_tme=0, pid=i + 2, ppid=1)
    ...
    def __iter__(self):
        ...
        yield Event.start(
            tme=0, pid=1, ppid=0, name=self._prototype.root().name)
        exit_event_queue.append(Event.exit(
            tme=0, pid=1, ppid=0, start_tme=0, name=root.name))
        picked = 0
        for i in range(self._tree_node_count):
            if random.random() <= self._relative_matching:
                ...
                pid = i + 2
            yield Event.start(
                tme=0, pid=pid, ppid=1, name=node_name)
            exit_event_queue.append(Event.exit(
                tme=0, pid=pid, ppid=1, name=node_name, start_tme=0))
