...
from assess.algorithms.distances.additionalmissingdistance import \
    AdditionalMissingDistance
from assess.algorithms.signatures.signatures import ParentChildByNameTopologySignature
from assess_tests.algorithms.distances.test_distance import monitoring_tree, \
    algorithm, additional_monitoring_tree
...
class TestAdditionalMissingDistance(unittest.TestCase):
    ...
    def test_creation(self):
        distance = AdditionalMissingDistance(
    ...
    def test_additional_distance(self):
        ...
        for index, node in enumerate(additional_monitoring_tree().nodes()):
            ...
            for result in distance._monitoring_results_dict:
                self.assertEqual(
                    list(result.values())[0], index * 2 if index <= 2 else 4)
        self.assertEqual(
            distance._monitoring_results_dict[0][self.algorithm.prototypes[0]], 4)
