...
class TestStartExitStuff(unittest.TestCase):
    def setUp(self):
        ...
        root = self._simple_prototype.add_node("root_node", tme=0, exit_tme=10, pid=2, ppid=1)
    ...
    def test_start_exit(self):
        ...
        alg = IncrementalDistanceAlgorithm(signature=signature, distance=StartExitDistance)
        ...
        alg.add_event(Event.exit(tme=3, start_tme=1, pid=3, ppid=2, name="first_child"))
        alg.add_event(Event.exit(tme=4, start_tme=1, pid=4, ppid=2, name="second_child"))
