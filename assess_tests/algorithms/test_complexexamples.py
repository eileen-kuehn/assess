...
class TestComplexExamples(unittest.TestCase):
    def test_attribute_distance(self):
        def distance_buidler(**kwargs):
            ...
            distance.supported = {ProcessStartEvent: True, ProcessExitEvent: True, TrafficEvent: True}
            return distance
    ...
    def test_initialisation_of_cluster_representatitives(self):
        ...
        distance.supported[TrafficEvent] = True
        signature = ParentChildByNameTopologySignature()
        tree_profiles = [tree_one.to_index(signature=signature,
        ...
        algorithm = IncrementalDistanceAlgorithm(signature=ParentChildByNameTopologySignature(),
                                                 distance=distance_builder, cache_statistics=SetStatistics)
