...
from assess_tests.basedata import simple_additional_monitoring_tree, \
    simple_prototype, real_tree
...
class TestPrototypeSignatureCache(unittest.TestCase):
    ...
    def test_from_signature_frequency(self):
        ...
        signature = ParentChildByNameTopologySignature()
        validation_index = prototype.to_index(
            signature=signature,
            cache=cache_builder()
        )
        prototype_index = PrototypeSignatureCache.from_signature_caches(
            [prototype.to_index(
                signature,
                cache=cache_builder()
            ) for _ in range(10)], prototype=prototype, threshold=0)
        self.assertEqual(
            validation_index.multiplicity(), prototype_index.multiplicity())
    ...
    def test_merging(self):
        ...
        self.assertEqual(
            one.node_count() + two.node_count(), prototype_one.multiplicity())
        ...
        self.assertEqual(
            one.node_count() * 2, prototype_three.multiplicity(prototype=one))
        self.assertEqual(
            two.node_count() * 2, prototype_four.multiplicity(prototype=two))
        self.assertEqual(0, prototype_three.multiplicity(two))
        prototype_three += prototype_four
        self.assertEqual(
            one.node_count() * 2, prototype_three.multiplicity(prototype=one))
        self.assertEqual(
            two.node_count() * 2, prototype_three.multiplicity(prototype=two))
        self.assertEqual(
            one.node_count() * 2 + two.node_count() * 2, prototype_three.multiplicity())
    ...
    def test_more_cluster_representatives(self):
        ...
        cr = PrototypeSignatureCache.from_signature_caches(
            signature_caches, prototype=tree_1)
        for token in cr:
            self.assertEqual(
                cr.multiplicity(token, tree_1), signature_caches[0].multiplicity(token))
            self.assertEqual(
                cr.get_statistics(
                    signature=token,
                    key="value",
